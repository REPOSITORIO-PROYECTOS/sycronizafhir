...
function Update-FromRelease {
    ...
    if (-not $asset) {
        throw "No se encontro asset '$AssetName' en release $($Release.tag_name)"
    }
...
try {
    ...
    if ($installedVersion -eq $latestVersion) {
        Write-Log "Sin cambios. Version actual: $installedVersion"
        exit 0
    }

    Write-Log "Nueva version detectada: $latestVersion (actual: $installedVersion)"
    ...
    Write-Log "Actualizacion completada a version $latestVersion"
}
catch {
