-- =============================================================================
-- PREPARACIÓN SUPABASE (una sola corrida recomendada)
-- =============================================================================
-- Dónde: Supabase Dashboard → SQL Editor → pegar todo este archivo → Run.
-- Base: postgres, schema public.
--
-- Mapa de archivos SQL del repo:
--
--   sql/000_supabase_prep_completo.sql  ← ESTE: tablas legacy en la NUBE
--        pedidos_d, pedidos_d_tor, productos, clientes
--        (PK + fecha_modificacion alineadas al dump mascotas / worker).
--
--   sql/001_sync_bridge_setup.sql       ← SOLO en PostgreSQL LOCAL (legacy)
...
--
-- Tablas que el worker puede sincronizar si existen en local con PK + fecha_modificacion
-- (ListSyncTables): en un dump típico mascotas aparecen también public.pedidos y
-- public.articulos. Este script NO crea public.pedidos ni public.articulos: si tu Supabase
-- ya tiene otras tablas con esos nombres (p. ej. productos con PK id), el upsert puede
-- fallar o hacer skip. Opciones:
--   - Excluir: variable SYNC_EXCLUDE_TABLES (coma-separada), o
--   - Crear tablas legacy con el mismo nombre/PK que en local (DDL aparte), o
--   - Schema distinto en Supabase (requeriría cambios en el bridge).
--
-- Realtime por defecto escucha public.pedidos: esa tabla debe existir en Supabase según
-- tu app; no forma parte de este DDL de detalle/clientes/productos.
--
-- Requisito del outbound/bootstrap: RLS/políticas o rol que permitan INSERT/UPDATE.
-- =============================================================================
